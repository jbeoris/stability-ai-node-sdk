...
export async function upscale(
  ...
  const [image, prompt, negative_prompt, output_format, seed, creativity] = args;
  ...
  if (negative_prompt) formData.negative_prompt = negative_prompt
  if (output_format) formData.output_format = output_format
  ...
  if (
    ...
    return {id: response.data.id, output_format: output_format || 'png'}
  }

  throw new StabilityAIError(response.status, 'Failed to start upscale', response.data);
}

export type UpscaleResultOptions = [
  id: string,
  output_format: OutputFormat
]

export type UpscaleResultResponse = Promise<
  StabilityAIContentResult | 
...
): UpscaleResultResponse
{
  ...
  if (response.status === 200) {
    ...
    const filename = `${uuidv4()}.${output_format || 'png'}`
...
export async function inpaint(
  ...
  const [modeOptions, image, prompt, negative_prompt, seed, output_format] = args;
  const imageFilepath = await Util.downloadImage(image)
  const maskFilepath = (modeOptions.mode === 'mask' && modeOptions.mask) ? 
    await Util.downloadImage(modeOptions.mask) : undefined
...
export async function imageToVideo(
  ...
  const [image, seed, motion_bucket_id, cfg_scale] = args;
  const imageFilepath = await Util.downloadImage(image)
  ...
  if (seed) formData.seed = seed
  if (motion_bucket_id) formData.motion_bucket_id = motion_bucket_id
  if (cfg_scale) formData.cfg_scale = cfg_scale
...
export type ImageToVideoResultOptions = [
  id: string
]

export type ImageToVideoResultResponse = Promise<
  StabilityAIContentResult | 
  {id: string, status: 'in-progress'}
>
